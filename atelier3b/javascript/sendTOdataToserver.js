...
$("#btn2").click(()=>{
		...
		var record =true;
		var data = "ident="+ident+"&maths="+maths+"&info="+info+"&record="+record;
		var url = "php/file_server.php";
		Send_data_to_server(url,data,record);
});
$(document).ready(()=>{
	$("#btn1").click(()=>{
		// event.preventDefault();
...
function Send_data_to_server(url_,data_,record){
	...
	window.location.assign("resultat_etd.html");
}